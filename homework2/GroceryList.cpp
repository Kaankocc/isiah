...
std::size_t GroceryList::size() const
{
  ...
  return _gList_vector.size();    // Easily picked this because vector's have a built in size factor.
  /////////////////////// END-TO-DO (1) ////////////////////////////
}
...
std::size_t GroceryList::find( const GroceryItem & groceryItem ) const
{
  ...
  {
    return it - _gList_vector.begin();    // Calculate position directly
  }
...
void GroceryList::insert( const GroceryItem & groceryItem, Position position )
{
  ...
  if( position == Position::TOP ) insert( groceryItem, 0 );
    ...
    throw std::logic_error( make_details( "Unexpected insertion position" ) );    // Programmer error.  Should never hit this!
}
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )    // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  {
    return;    // Item already exists; silently discard
  }
  /////////////////////// END-TO-DO (3) ////////////////////////////
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ...
    {
      throw CapacityExceeded_Ex( "Cannot insert; capacity exceeded." );
    }
    ...
    ++_gList_array_size;    // Update the size
  ...
  { /**********  Part 4 - Insert into singly linked list  **********/
    ...
    auto it_sll = std::next( _gList_sll.before_begin(), offsetFromTop );
    _gList_sll.insert_after( it_sll, groceryItem );
...
void GroceryList::moveToTop( const GroceryItem & groceryItem )
{
  ...
  // Check if the item was found (position should be less than size)
  if( position < size() )
  {
